...
---
In [part 1]({% link _posts/2025-04-26-Improving-Grampsweb-kubernetes-setup-with-a-Postgres-database-part-1.markdown %}) we configured `Grampsweb` to use external `PostgreSQL` databases for the `Users`and `Search index`. In this part we will replace the default `SQLight` database for the `family tree`.
